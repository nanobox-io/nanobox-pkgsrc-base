...
###
.if !defined(IS_BUILTIN.sun-jre8)
.  if empty(JAVAVM8:M__nonexistent__)
IS_BUILTIN.sun-jre8=	yes
.  else
IS_BUILTIN.sun-jre8=	no
.  endif
.endif
MAKEVARS+=	IS_BUILTIN.sun-jre8

###
### If there is a built-in implementation, then set BUILTIN_PKG.<pkg> to
### a package name to represent the built-in package.
###
.if !defined(BUILTIN_PKG.sun-jre8) && \
    !empty(IS_BUILTIN.sun-jre8:M[yY][eE][sS]) && \
    empty(JAVAVM8:M__nonexistent__)

BUILTIN_VERSION.sun-jre8!= ${JAVAVM8:Q}/bin/java -version 2>&1 | \
	${AWK} -F \" '{print $$2; exit}' | \
	${AWK} '{sub(/^1\./,"");sub(/_/,".");print $$1}'

BUILTIN_PKG.sun-jre8=	sun-jre8-${BUILTIN_VERSION.sun-jre8}
.endif
MAKEVARS+=	BUILTIN_PKG.sun-jre8

###
### Determine whether we should use the built-in implementation if it
### exists, and set USE_BUILTIN.<pkg> appropriate ("yes" or "no").
###
.if !defined(USE_BUILTIN.sun-jre8)
.  if ${PREFER.sun-jre8} == "pkgsrc"
USE_BUILTIN.sun-jre8=	no
.  else
USE_BUILTIN.sun-jre8=	${IS_BUILTIN.sun-jre8}
.    if defined(BUILTIN_PKG.sun-jre8) && \
        !empty(IS_BUILTIN.sun-jre8:M[yY][eE][sS])
USE_BUILTIN.sun-jre8=	yes
.      for _dep_ in ${BUILDLINK_API_DEPENDS.sun-jre8}
.        if !empty(USE_BUILTIN.sun-jre8:M[yY][eE][sS])
USE_BUILTIN.sun-jre8!=						\
	if ${PKG_ADMIN} pmatch ${_dep_:Q} ${BUILTIN_PKG.sun-jre8:Q}; then \
		${ECHO} yes;						\
	else								\
		${ECHO} no;						\
	fi
...
.      endfor
.    endif
.  endif  # PREFER.sun-jre8
.endif
MAKEVARS+=	USE_BUILTIN.sun-jre8
...
.if !empty(CHECK_BUILTIN.sun-jre8:M[nN][oO])
.  if !empty(USE_BUILTIN.sun-jre8:M[yY][eE][sS])
